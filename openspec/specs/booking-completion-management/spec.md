...
The system SHALL allow authenticated tenant users to complete a confirmed booking belonging to their authenticated `Empresa`. A completed booking becomes eligible for feedback submission when the client later presents the valid booking-backed feedback access token and no feedback has yet been stored.
...
- **THEN** the system marks the booking as `completed`, stores the final charged price, and makes the booking eligible for later feedback submission
