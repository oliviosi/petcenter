## Purpose
Define public booking request intake and asynchronous booking lifecycle requirements.
...
- **THEN** the system stores a booking request in `requested` state with a generated booking identifier, returns that initial lifecycle state immediately, and issues the feedback access mechanism needed for a later post-service feedback submission
