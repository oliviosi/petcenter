...
#### Scenario: Fallback canonical link returns after degradation
- **WHEN** a previously active custom domain loses readiness after activation
- **THEN** the system shows the shared-host storefront link as the canonical public entry until the custom domain becomes fully ready again

### Requirement: Tenant can understand storefront link availability state
The system SHALL communicate whether the canonical storefront link is active, preview-only, or unavailable based on publication readiness, slug availability, and staged custom-domain readiness, and SHALL keep the shared-host fallback as the active canonical link while DNS verification or certificate readiness is still pending or has most recently failed, regardless of whether the desired domain is apex/root or subdomain, including when the failure occurs after a domain was already active.
