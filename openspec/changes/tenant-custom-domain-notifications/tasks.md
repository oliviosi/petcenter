...
2. [backend] Implement `INotificationService` and `EmailNotificationProvider` wiring to existing email infra.
3. [backend] Subscribe NotificationService to `tenant.domain.status.changed` events and implement deduplication logic.
