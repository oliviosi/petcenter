...
public static class DataSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        try
        {
            ...
            {
                // If services exist, ensure each service has at least one professional assignment and that professional has availability.
                foreach (var servico in existingServices)
                {
                    var hasAssignmentForService = await db.ProfessionalServiceAssignments.AnyAsync(a => a.ServiceId == servico.Id);
                    if (!hasAssignmentForService)
                    {
                        ...
                        // ensure availability exists for this professional
                        ...
                        db.ProfessionalServiceAssignments.Add(assignment);
                        await db.SaveChangesAsync();
                        logger.LogInformation($"Linked service {servico.Id} to professional {profissional.Id} and ensured availability for development.");
                    }
                }
            }
        }
